...
from lhotse.dataset import SingleCutSampler, VadDataset
...
def decode(
    ...
    for batch_idx, batch in enumerate(dataloader):
        ...
        sequence_idx= torch.Tensor(range(feature.shape[0]))
        start_frame= torch.zeros(feature.shape[0])
        duration= torch.Tensor(
                    [batch["is_voice"].shape[1] for _ in range(batch["is_voice"].shape[0])])
        ...
        supervision_segments = torch.stack(
            (
                sequence_idx,
                torch.floor_divide(
                    start_frame, model.subsampling_factor
                ),
                torch.floor_divide(duration, model.subsampling_factor),
            ),
            1,
        ).to(torch.int32)
        ...
        assert best_paths.shape[0] == supervisions.shape[0]

        # best_paths is an FsaVec, and each of its FSAs is a linear FSA
        references = supervisions[indices]
        for i in range(references.shape[0]):
            ...
            results.append((batch["cut"][indices[i]], ref, hyp))
...
def main():
    ...
    cuts_test = CutSet.from_json(feature_dir / "cuts_dihard_dev.json.gz")
    tot_cut = len(list(cuts_test))
    logging.info("About to create test dataset")
    test = VadDataset()
    test_sampler = SingleCutSampler(cuts_test, max_frames=100000)
    logging.info("About to create test dataloader")
    test_dl = torch.utils.data.DataLoader(
        test, batch_size=None, sampler=test_sampler, num_workers=4
    )
    ...
    results = decode(dataloader=test_dl, model=model, device=device, HCLG=HCLG, tot_cut=tot_cut)
