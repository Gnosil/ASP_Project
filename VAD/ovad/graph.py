...
def prepare_decoding_graph(
    ...
    onset_score: Optional[float] = 0.5,
    offset_score: Optional[float] = 0.5,
) -> Fsa:
    """
    ...
    for state in range(
        ...
        transitions.append(
            Transition(src=state, dest=state + 1, label=silence, score=offset_score)
        )
    silence_last_state = silence_start_state + min_states_silence - 1

    # Self-loop that accpet all but gives a weight to silence
    transitions.append(
        Transition(
            src=silence_start_state, dest=silence_last_state, label=silence, score=offset_score
        )
    )
    ...
    transitions.append(
        Transition(
            ...
            dest=speech_start_state,
            label=speech,
            score=-math.log(1.0-offset_score),
        )
    )
    ...
    for state in range(speech_start_state, speech_start_state + min_states_speech - 1):
        transitions.append(
            Transition(src=state, dest=state + 1, label=speech, score=onset_score)
        )
    ...
    transitions.append(
        Transition(
            src=speech_last_state,
            dest=silence_start_state,
            label=silence,
            score=-math.log(1.0 - onset_score),
        )
    )
    ...
    text_file = open("/export/c07/sli218/lhotse/ovad/scripts/Dihard3/transition.txt", "w")
    n = text_file.write(transitions)
    text_file.close()
